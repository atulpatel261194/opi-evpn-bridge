...
func (neigh NeighStruct) neighborAnnotate() NeighStruct {
	...
	if strings.HasPrefix(nameIndex[neigh.Neigh0.LinkIndex], path.Base(neigh.VrfName)) && neigh.Protocol != zebraStr {
		...
		if lb == nil {
			log.Printf("ERROR: No matching logical bridge found for vlanid:%+v", vlanID)
		}
		if lb.Spec.Vni != nil {
			...
			fdbEntry, ok := latestFDB[FdbKey{vid, neigh.Neigh0.HardwareAddr.String()}]
			if ok {
				neigh.Metadata["l2_nh"] = fdbEntry.Nexthop
				neigh.Type = VXLAN // confirm this later
			} else {
				log.Printf("DEBUG neighborAnnotate: FDB entry not found for VLAN %+v", vid)
			}
		}
