...
func dumpRouteDB() string {
	...
	for _, n := range routes {
		...
		for _, nh := range n.Nexthops {
			//nhIDs += string(rune(nh.ID))
			nhIDs += strconv.Itoa(nh.ID) // Convert integer to string
			nhIDs += ","
		}
		str := fmt.Sprintf("Route(vrf=%s dst=%s type=%s proto=%s metric=%d  via=%s dev=%s nhid= %+v Table= %d)", n.Vrf.Name, n.Route0.Dst.String(), n.NlType, n.getProto(), n.Route0.Priority, via, nameIndex[n.Route0.LinkIndex], nhIDs, n.Route0.Table)
		s += str
		s += "\n"
	}
	s += "\n\n"
